...
#  define STRING_MANUFACTURER "Pete Jansen"
#endif

#ifndef STRING_PRODUCT
#  define STRING_PRODUCT "9DOF Logger"
#endif

extern void software_reset_mcu(void);

void software_reset_mcu(void)
{
...
void sam_ba_usb_CDC_Enumerate(P_USB_CDC pCdc)
{
  ...
  //xprintf("USB CDC request 0x%02x type 0x%02x wvalue %d len %d\n", bRequest, bmRequestType, wValue, wLength);
  ...
  switch ((bRequest << 8) | bmRequestType)
  {
      ...
      if ((wValue == 0) && wIndex && (wIndex <= 3))
      {
        if (dir)
        {
          if (pUsb->DEVICE.DeviceEndpoint[wIndex].EPSTATUS.bit.STALLRQ0 & (1<<1))
          {
            ...
            pUsb->DEVICE.DeviceEndpoint[wIndex].EPSTATUSCLR.bit.STALLRQ0 = (1<<1);
            if (pUsb->DEVICE.DeviceEndpoint[wIndex].EPINTFLAG.bit.STALL0 & (1<<1))
            {
              pUsb->DEVICE.DeviceEndpoint[wIndex].EPINTFLAG.bit.STALL0 = (1<<1);
    ...
    case SET_LINE_CODING:
    	  // need to do something else to get the line coding?????
    	  /* Send ZLP */
      USB_SendZlp(pUsb);
    break;
    ...
    case SET_CONTROL_LINE_STATE:
      ...
      // detect arduino bootloader reset
